<template>
  <div class="user-info">
    <!-- 个人信息 -->
    <el-form label-width="220px" :model="userInfo" :rules="rulesForm">
      <!-- 工号 入职时间 -->
...
export default {
  ...
  methods: {
    ...
    async getPersonalDetail() {
      ...
      if (this.formData.myStaffPhoto && (this.formData.myStaffPhoto.trim())) {
    ...
    async saveUser() {
      ...
      if (flag) {
        // 说明此时由图片未上传完成
        return this.$message.warning('此时图片还未上传完成')
      }
      //  合并 staffPhoto由于接口问题  必须给一个有空格的字符串才可以
